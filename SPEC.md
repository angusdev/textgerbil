...
- **slide**: CodeMirror-backed Markdown slide deck. The mode only uses Markdown (`language: "markdown"`), and each level-one heading (`#`) starts a new slide. The preview button renders the deck as a vertical scroll list of sandboxed slide iframes. The present button opens a focused presentation overlay that advances one slide at a time with buttons or keyboard arrows. Slide bodies render at the standard content font size, then an isolated nonce-bearing fitting script measures the available page height against the rendered `.slide-content` height and writes a CSS `zoom` value so the body avoids scrollbars. Preview thumbnails always run this fitting pass against their preview-frame height, including slides with `fit-to-page: false`; full slide rendering honors `fit-to-page: false` unless fitting is explicitly forced by the caller. Slide iframes are sandboxed and use a strict CSP that allows inline images from `data:`, `https:`, and `http:` sources plus only the generated fitting script.
