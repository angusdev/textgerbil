...
(async function () {
  ...
  dom.window.addEventListener('load', async () => {
    try {
      ...
      assert(slideFrames[0].getAttribute('sandbox') === 'allow-scripts', 'Slide preview iframe allows isolated fitting scripts');
      assert(firstSlideSrcdoc.includes('img-src data: https: http:'), 'Slide CSP supports data-url and linked images');
      assert(firstSlideSrcdoc.includes("script-src 'nonce-"), 'Slide CSP only allows the nonce-bearing fitting script');
      ...
      slideTab.content = '<!--\nfit-to-page: false\n-->\n# Preview fit\n\n' + 'Dense preview content '.repeat(140);
      w.__textgerbil.updatePreview();
      const noFitPreviewSrcdoc = doc.querySelector('#previewContent .slide-preview-frame').getAttribute('srcdoc') || '';
      assert(noFitPreviewSrcdoc.includes('content.style.zoom'), 'Slide preview still fits fit-to-page:false slides to the preview frame');
      assert(noFitPreviewSrcdoc.includes('pageHeight/contentHeight'), 'Slide preview fit uses the preview slide height for fit-to-page:false slides');
      ...
      assert(denseSlideSrcdoc.includes('font-size:22px'), 'Slide body renders at the standard content font size');
      assert(denseSlideSrcdoc.includes('class="slide-content"'), 'Slide body content renders in the zoomable slide-content element');
      assert(denseSlideSrcdoc.includes('content.style.zoom'), 'Slide content zoom is set from measured layout height');
      assert(denseSlideSrcdoc.includes('pageHeight/contentHeight'), 'Slide content zoom is calculated from page and content heights');
      assert(!denseSlideSrcdoc.includes('--body-scale'), 'Slide body scale is not precomputed before rendering');
      assert(!w.__textgerbil.buildSandboxedSlideSrcdoc('<!--\nfit-to-page: false\n-->\n# No fit\n\nBody').includes('content.style.zoom'), 'Slide srcdoc honors fit-to-page:false unless preview forces fitting');
      slideTab.content = '# Intro\n\n![Tiny](data:image/png;base64,abc)\n\n[Site](https://example.com)\n\n# Second\n\nMore text';
      ...
      assert(presentFrame && presentFrame.getAttribute('sandbox') === 'allow-scripts', 'Present slide iframe allows isolated fitting scripts');
